...
class WordScoringStrategy(ScoringStrategy):
    ...
    def _possible_words(self, sentence: str) -> list:
        return [sentence[word_start:word_end].lower() for word_start in range(len(sentence)-1) for word_end in range(word_start, len(sentence))]
...
class PlainTextGuesser():
    ...
    def most_likely_plaintext(self, candidates: list) -> dict:
        ordered_plaintexts = SortedDict(((self._scoring_strategy.score(plaintext), (key, plaintext)) for key, plaintext in candidates.items()))
