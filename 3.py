...
from lib.scoring import PlainTextGuesser, ScoringStrategy, WordCountScoringStrategy, WordLengthScoringStrategy

def decipher_with_every_key(ciphertext: str) -> dict:
    return { chr(key): sxorc(ciphertext, key) for key in range(ord('a'), ord('z')) }

def guess_plaintext(possible_plaintexts: list, scoring_strategy: ScoringStrategy) -> None:
    guesser = PlainTextGuesser(scoring_strategy)
...
guess_plaintext(possible_plaintexts, WordCountScoringStrategy('words.txt'))

guess_plaintext(possible_plaintexts, WordLengthScoringStrategy('words.txt'))
